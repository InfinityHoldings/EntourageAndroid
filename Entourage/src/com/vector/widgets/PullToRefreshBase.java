...
import com.vector.widgets.IPullToRefresh;
import com.vector.widgets.LoadingLayoutProxy;
import com.vector.widgets.OverscrollHelper;
import com.vector.widgets.FlipLoadingLayout;
import com.vector.widgets.LoadingLayout;
import com.vector.widgets.RotateLoadingLayout;
import com.vector.widgets.Utils;
import com.vector.widgets.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	public final boolean demo() {
		 ...
		 * smoothScrollToAndBack(getFooterSize() * 2); return true; }
		 */
		return false;
	}
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			// If we're refreshing, and the flag is set.
			...
			if (isReadyForPull()) {
				...
				if (absDiff > mTouchSlop
					 ...
					 * && isReadyForPullEnd()) { mLastMotionY = y; mLastMotionX
					 * = x; mIsBeingDragged = true; if (mMode == Mode.BOTH) {
	 ...
	 * getLoadingLayoutProxy(mode.showHeaderLoadingLayout(),
	 * mode.showFooterLoadingLayout()).setRefreshingLabel( refreshingLabel); }
	 */

	public void setReleaseLabel(CharSequence releaseLabel) {
		// setReleaseLabel(releaseLabel, Mode.BOTH);
		getLoadingLayoutProxy().setReleaseLabel(releaseLabel);
	}
	...
	// Used internally for getLoadingLayoutProxy(boolean, boolean)
	...
	// Called by onRestoreInstanceState(Parcelable) so that derivative
	...
	// PULL_TO_REFRESH state.
	protected void onPullToRefresh() {
		switch (mCurrentMode) {
		/*
		 * case PULL_FROM_END: mFooterLayout.pullToRefresh(); break;
		 */
	...
	// Called when the UI has been to be updated to be in the
	// REFRESHING or MANUAL_REFRESHING state.
	protected void onRefreshing(final boolean doScroll) {
		if (mMode.showHeaderLoadingLayout()) {
			mHeaderLayout.refreshing();
		}
		/*
		 * if (mMode.showFooterLoadingLayout()) { mFooterLayout.refreshing(); }
		 */
		...
		if (doScroll) {
			if (mShowViewWhileRefreshing) {
				...
				case MANUAL_REFRESH_ONLY:
					/*
					 * case PULL_FROM_END: smoothScrollTo(getFooterSize(),
					 * listener); break;
					 */
				default:
				case PULL_FROM_START:
	...
	// Called when the UI has been to be updated to be in the
	// RELEASE_TO_REFRESH} state.
	protected void onReleaseToRefresh() {
		switch (mCurrentMode) {
		/*
		 * case PULL_FROM_END: mFooterLayout.releaseToRefresh(); break;
		 */
		case PULL_FROM_START:
	...
	protected final void refreshLoadingViewsSize() {
		...
		case HORIZONTAL:
			 ...
			 * mFooterLayout.setWidth(maximumPullScroll); pRight =
			 * -maximumPullScroll; } else { pRight = 0; }
			 */
			break;
		...
		case VERTICAL:
			...
			/*
			 * if (mMode.showFooterLoadingLayout()) {
			 * mFooterLayout.setHeight(maximumPullScroll); pBottom =
			 * -maximumPullScroll; } else { pBottom = 0; }
			 */
			break;
		}
	...
	protected final void setHeaderScroll(int value) {
		...
		if (mLayoutVisibilityChangesEnabled) {
			 ...
			 * else if (value > 0) { mFooterLayout.setVisibility(View.VISIBLE);
			 * }
			 */else {
	...
	protected void updateUIForMode() {
		 ...
		 * if (mMode.showFooterLoadingLayout()) { addViewInternal(mFooterLayout,
		 * lp); }
		 */
	...
	public static enum Mode {
		...
		/*
		 * public boolean showFooterLoadingLayout() { return this ==
		 * PULL_FROM_END || this == BOTH || this == MANUAL_REFRESH_ONLY; }
		 */
	...
	public static enum State {
		...
		// When the UI is currently refreshing
